...
@RequestMapping(value= {"/category/list","/category/list/"})
	...
	public String add(Model model) {
		model.addAttribute("titleCategory", "Add Category");
	...
	public String edit(Model model , @PathVariable("id") int id) {
		...
		if(category != null) {
			model.addAttribute("titleCategory", "Edit category");
			model.addAttribute("modelForm", category);
			model.addAttribute("viewOnly", false); //Dung cho view mode
			return "category-action";
		}else {
	...
	public String view(Model model , @PathVariable("id") int id){
		...
		Category category = productService.findByIdCategory(id);
		if(category != null) {
			model.addAttribute("titleCategory", "View category");
			model.addAttribute("modelForm", category);
			model.addAttribute("viewOnly", true); //Dung cho view mode
			return "category-action";
		}else {
			return "redirect:/category/list";
		}
	...
	public String save(Model model,@ModelAttribute("modelForm") @Validated Category category,BindingResult result, HttpSession session) {
		...
		if(category.getId()!=null && category.getId()!=0) {
			...
			try {
				...
				e.printStackTrace();
				session.setAttribute(Constant.MSG_ERROR, "Insert success!!!");
				log.error(e.getMessage());
			}
